...
export default function useFetchIncomesExpenses(
    dateRange: any = null,
...
) 
{
    ...
    useEffect(() => {
        if (budgetId === undefined)
            return;
        setLoading(true);
        axios.get(process.env.REACT_APP_API_URL + `/api/insights/${budgetId}/income-expenses/${query}`, {
            headers: {
                'Accept': '*',
        ...
        .then(res => {
            setData([{income: res.data.income, expenses: -res.data.expenses}]);
        })
        .catch(error => {
            setError(true);
            console.error(error);
